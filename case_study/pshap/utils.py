...
def check_convergence(mem):
    ...
    percent = 25
    while True:
        IQR_constant = np.percentile(mem.reshape(-1), 50 + percent) - np.percentile(mem.reshape(-1), 50 - percent)
        if IQR_constant == 0:
            percent += 10
            if percent == 105:
    ...
    for j in range(n_to_be_valued):
        ...
        mem_tmp_j = mem_tmp_j_original / IQR_constant
...
def point_removal_exp(shap_engine, X_test, y_test):
    ...
    remove_dict['random'] = np.mean([perf_func(np.random.permutation(shap_engine.n_sources)) for _ in range(10)],
                                    axis=0)
