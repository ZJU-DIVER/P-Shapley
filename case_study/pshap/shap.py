...
def aswish(x):
    return math.exp(x) - 1 #  * (1 / (1 + math.exp(-1 * x)))

def alog(x):
    return (x+1) * np.log2(x+1)
...
class ShapEngine(object):
    ...
    def _initialize_instance(self):
        if self.problem == 'regression':
            ...
            if self.metric not in ['accuracy','f1','auc','likelihood', 'likelihood_indicator', 'loss']:
    ...
    def init_score(self):
        ...
        if self.problem == 'regression':
            ...
            if self.metric == 'accuracy':
                ...
                # hist = np.bincount(self.y_val)/len(self.y_val)
                # return np.sum(hist*(hist+1e-16))
                return 0.5
            elif self.metric == 'likelihood_indicator':
                return 0.25
    ...
    def value(self, X=None, y=None):
        ...
        if self.problem == 'regression':
            ...
            if self.metric == 'accuracy':
                ...
                return np.mean(true_probs)
            elif self.metric == 'likelihood_indicator':
                probs = self.model.predict_proba(X)
                true_probs = probs[np.arange(len(y)), y]
                try:
                    for k in range(len(true_probs)):
                        if true_probs[k] < 0.5:
                            true_probs[k] = 0.0
                        if true_probs[k] == 0.5:
                            true_probs[k] = 0.25
    ...
    def _calculate_marginal_contributions(self, model_name='model_name'):
        ...
        self.act_prob_marginal_contrib=np.zeros((self.n_sources, self.n_sources))
        self.act_prob_marginal_contrib_count=np.zeros((self.n_sources, self.n_sources))
        self.act_prob_marginal_contrib_mean=np.zeros((0, self.n_sources))

        self.prob_GR_dict=dict()

        for iters in trange(10*self.max_iters):
        ...
        self.act_MC_obs_card_mat=self.act_prob_marginal_contrib
        self.act_MC_count_obs_card_mat=self.act_prob_marginal_contrib_count
    ...
    def _calculate_marginal_contributions_core(self):
        ...
        for _ in range(100):
            ...
            act_marginal_contribs_tmp=np.zeros(self.n_sources)
            ...
            for n, idx in enumerate(idxs):
                ...
                act_marginal_contribs_tmp[idx] = (self.act(new_score)-self.act(old_score))*self.weight_list[n]
                ...
                self.act_prob_marginal_contrib[idx, n]+=act_marginal_contribs_tmp[idx]
                self.act_prob_marginal_contrib_count[idx, n]+=1
